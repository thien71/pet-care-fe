...
import { FaHome, FaStore, FaCalendarAlt, FaHistory, FaSearch, FaUser, FaSignOutAlt, FaBars, FaTimes } from "react-icons/fa";
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center gap-3 hover:opacity-80 transition-opacity">
            <Logo className="mb-0! p-2!" iconClassName="text-2xl!" animate={false} />
            ...
            {menuItems.map((item) => (
              <Link
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-colors ${
                  isActive(item.path) ? "bg-[#8e2800] text-white" : "text-gray-700 hover:bg-gray-100 hover:text-[#8e2800]"
                }`}
              >
                {item.icon}
          ...
          <div className="flex items-center gap-3">
            {isAuthenticated ? (
              <>
                ...
                <div ref={userMenuRef} className="relative">
                  ...
                  >
                    ...
                    {avatarUrl ? (
                      <img
                        ...
                        onError={(e) => {
                          ...
                          fallback.textContent = user?.hoTen?.charAt(0).toUpperCase() || "?";
                    ...
                    <span className="hidden lg:block font-medium text-gray-700">{user?.hoTen || "User"}</span>
                  </button>
              ...
              <div className="flex items-center gap-2">
                <Link to="/login" className="px-4 py-2 text-gray-700 hover:text-[#8e2800] font-medium transition-colors">
                  Đăng nhập
                </Link>
                <Link to="/register" className="px-4 py-2 bg-[#8e2800] text-white rounded-lg hover:bg-[#6d1f00] transition-colors">
                  Đăng ký
                </Link>
              </div>
            )}
            ...
            >
              {showMobileMenu ? <FaTimes className="text-xl text-gray-600" /> : <FaBars className="text-xl text-gray-600" />}
            </button>
          </div>
        </div>
