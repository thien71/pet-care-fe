      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <div className="card w-96 bg-white shadow-xl">
          <div className="card-body items-center text-center">
            ...
            <button onClick={() => navigate("/")} className="btn mt-4 text-white" style={{ backgroundColor: "#8e2800" }}>
              Về trang chủ
            </button>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 mb-16">
            ...
            <div className="bg-gray-200 rounded-2xl overflow-hidden shadow-lg h-96 flex items-center justify-center sticky top-20">
              {service.shop.anhCuaHang ? (
                ...
                <div className="text-center">
                  ...
                  <p className="text-gray-500 font-semibold">{service.tenDichVu}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-3 space-y-6">
            {/* Service Name & Rating */}
            <div>
              <h1 className="text-4xl font-bold text-gray-900 mb-3">{service.tenDichVu}</h1>
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-1">
                  <span className="text-yellow-500 text-lg">⭐</span>
                  <span className="text-2xl font-bold text-gray-900">{service.rating || "N/A"}</span>
                  <span className="text-gray-600 text-sm">({service.reviewCount || 0} đánh giá)</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
              ...
              <div className="bg-linear-to-br from-gray-50 to-gray-100 rounded-xl p-5 shadow-sm">
                <p className="text-gray-600 text-xs uppercase tracking-wide mb-1">Giá dịch vụ</p>
              ...
              {service.thoiLuong && (
                <div className="bg-linear-to-br from-gray-50 to-gray-100 rounded-xl p-5 shadow-sm">
                  <p className="text-gray-600 text-xs uppercase tracking-wide mb-1">Thời lượng</p>
                  <div className="flex items-baseline gap-1">
                    <span className="text-3xl font-bold text-gray-900">{service.thoiLuong}</span>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-md">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div>
                ...
                <p className="text-lg font-bold text-gray-900 line-clamp-2">{service.shop.tenCuaHang}</p>
              </div>
              <div>
                <p className="text-gray-600 text-sm mb-1">📍 Địa chỉ</p>
                <p className="text-sm text-gray-700 line-clamp-2">{service.shop.diaChi}</p>
              </div>
            ...
            {service.shop.moTa && (
              <div className="mt-4 pt-4 border-t border-gray-200">
                <p className="text-gray-700 text-sm">
                  <span className="font-semibold">Giới thiệu:</span> {service.shop.moTa}
                </p>
              </div>
            )}
          </div>
        ...
        <div className="mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-gray-50 rounded-2xl p-6 border border-gray-200 text-center">
              <p className="text-5xl font-bold mb-2" style={{ color: "#8e2800" }}>
                {service.rating || "N/A"}
              </p>
              <div className="text-2xl text-yellow-500 mb-2">
                {Array(5)
                  .fill(0)
                  .map((_, i) => (
                    <span key={i}>{i < Math.round(service.rating || 0) ? "⭐" : "☆"}</span>
                  ))}
              </div>
              <p className="text-gray-600">{service.reviewCount || 0} đánh giá</p>
            </div>
            <div className="md:col-span-2 bg-gray-50 rounded-2xl p-6 border border-gray-200">
              ...
              {[5, 4, 3, 2, 1].map((star) => (
                <div key={star} className="flex items-center gap-3 mb-2">
                  <span className="w-12 text-sm font-semibold text-gray-600">{star} ⭐</span>
                  <progress className="progress progress-warning flex-1" value={star * 20} max="100" />
                  <span className="text-xs text-gray-500 w-12 text-right">{Math.floor(Math.random() * 30)}%</span>
                </div>
              ))}
            </div>
          </div>
                ...
                <div key={idx} className="bg-white rounded-xl border border-gray-200 p-6">
                  <div className="flex items-start gap-4">
                    <div className="avatar placeholder">
                      <div className="rounded-full w-12 text-white" style={{ backgroundColor: "#8e2800" }}>
        ...
        {service.otherServices && service.otherServices.length > 0 && (
          <div className="mb-16">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">✨ Các Dịch Vụ Khác Của Cửa Hàng</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {service.otherServices.map((s) => (
                ...
                >
                  ...
                  <div className="p-4">
                    <h3 className="font-bold text-gray-900 line-clamp-2 mb-2">{s.tenDichVu}</h3>
                    <div className="flex justify-between items-center">
                      <span className="font-bold text-lg" style={{ color: "#8e2800" }}>
                        {Number.parseInt(s.gia).toLocaleString("vi-VN")}đ
                      </span>
                      {s.thoiLuong && <span className="text-xs text-gray-500">⏱️ {s.thoiLuong}p</span>}
                    </div>
                  </div>
                </Link>
              ))}
