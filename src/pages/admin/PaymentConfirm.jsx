...
const PaymentConfirm = () => {
  ...
  const handleConfirm = async (paymentId) => {
    if (window.confirm("Xác nhận thanh toán cho cửa hàng này?")) {
      try {
        ...
        // await apiClient.put(`/admin/payment-confirmations/${paymentId}/confirm`);
  ...
  const handleReject = async (paymentId) => {
    if (window.confirm("Từ chối thanh toán này? Cửa hàng sẽ cần thanh toán lại.")) {
      try {
        setLoading(true);
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-2">Xác nhận các khoản thanh toán từ cửa hàng</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="stat bg-base-100 shadow rounded-lg">
          ...
          <div className="stat-value text-warning">{payments.filter((p) => p.trangThai === "CHUA_THANH_TOAN").length}</div>
        </div>
        <div className="stat bg-base-100 shadow rounded-lg">
          <div className="stat-title">Đã thanh toán</div>
          <div className="stat-value text-success">{payments.filter((p) => p.trangThai === "DA_THANH_TOAN").length}</div>
        </div>
        <div className="stat bg-base-100 shadow rounded-lg">
          <div className="stat-title">Quá hạn</div>
          <div className="stat-value text-error">{payments.filter((p) => p.trangThai === "QUA_HAN").length}</div>
        </div>
      </div>
              ...
              {payments.length > 0 ? (
                payments.map((payment) => (
                  <tr key={payment.maThanhToan} className="hover">
                    <td>
                      <div className="font-semibold">{payment.CuaHang?.tenCuaHang || "N/A"}</div>
                    </td>
                    <td>
                      <div>{payment.GoiThanhToan?.tenGoi || "N/A"}</div>
                      <div className="text-xs text-gray-500">Gói {payment.GoiThanhToan?.thoiGian} tháng</div>
                    </td>
                    <td>
                    ...
                    <td className="space-x-2">
                      {payment.trangThai === "CHUA_THANH_TOAN" && (
                        <>
                          <button onClick={() => handleConfirm(payment.maThanhToan)} className="btn btn-sm btn-success">
                            ✅
                          </button>
                          <button onClick={() => handleReject(payment.maThanhToan)} className="btn btn-sm btn-error">
                            ❌
                          </button>
                        </>
                      )}
                      {payment.trangThai === "DA_THANH_TOAN" && <span className="text-success">✅ Đã xác nhận</span>}
                    </td>
                  </tr>
                ))
              ) : (
